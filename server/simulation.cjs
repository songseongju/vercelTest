...
class Battle {
  ...
  action(id,type,data){const p=this.players.get(id);if(!p||!p.connected)return false;
    ...
    if(type==='jump')return W.jump(p);
    if(type==='stance')return W.changeStance(p,data);
    if(type==='collect'){if(this.now-p.collectAt<.15)return false;p.collectAt=this.now;let closest=null,distance=3.1;for(const item of this.loot){if(item.taken)continue;const d=Math.hypot(item.x-p.x,item.z-p.z);if(d<distance&&W.visible({x:p.x,y:W.eyeHeight(p),z:p.z},{x:item.x,y:.45,z:item.z})){closest=item;distance=d}}const old=p.equipped;if(closest&&R.collect(p,closest)){if(old!==p.equipped)p.reload=p.heal=0;this.events.push({type:'collect',id:p.id,item:closest.id});return true}return false}
    ...
    if(type==='throw'){if(!data||!R.throwables[data.kind]||!Number.isFinite(data.yaw)||!Number.isFinite(data.pitch)||Math.abs(data.yaw)>1e6)return false;
      ...
      const spec=R.throwables[data.kind],g=W.throwGrenade(data.kind,p.x,W.eyeHeight(p)-.15,p.z,p.yaw,p.pitch,spec.fuse);
  ...
  shoot(p){const w=R.weapons[p.equipped],slot=p.weapons[p.equipped];if(!w||p.cooldown>0||p.reload>0)return;if(!w.melee){if(!slot)return;if(slot.ammo<=0){this.action(p.id,'reload');return}slot.ammo--}p.cooldown=w.interval;p.heal=0;const origin={x:p.x,y:W.eyeHeight(p),z:p.z},cos=Math.cos(p.pitch),dir={x:Math.sin(p.yaw)*cos,y:-Math.sin(p.pitch),z:Math.cos(p.yaw)*cos};let distance=W.wallDistance(origin,dir,w.range),victim=null,head=false;
    for(const q of this.players.values()){if(q===p||q.hp<=0)continue;const volumes=W.hitVolumes(q),body=W.rayBox(origin,dir,volumes.body,distance),top=W.rayBox(origin,dir,volumes.head,distance);const hit=Math.min(body,top);if(hit<distance){distance=hit;victim=q;head=top<body}}
