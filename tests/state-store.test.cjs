...
test('disconnect expiry removes abandoned players and joins never exceed sixteen',()=>{let s=makeState('a','A','a'.repeat(48),'a',1000);const commands=Array.from({length:17},(_,i)=>({op:'join',id:'p'+i,name:'P',hash:'hash',owner:'o'+i,request:'r'+i,at:1100}));s=evolve(s,commands,1100);assert.equal(Battle.restore(s.game).players.size,16);assert.equal(s.receipts.r14.ok,true);assert.equal(s.receipts.r15.ok,false);s=evolve(s,[],22000);assert.equal(Battle.restore(s.game).players.size,0);});
