...
function newPlayer(){return{hp:100,armor:0,kits:0,reserve:0,weapons:{},equipped:null,kills:0}}
function collect(p,item){if(item.taken)return false;if(item.type==='weapon'){const w=weapons[item.weapon];if(!w)return false;if(p.weapons[item.weapon])p.reserve+=w.mag;else{p.weapons[item.weapon]={ammo:w.mag};p.equipped=item.weapon}}else if(item.type==='ammo')p.reserve+=item.amount||60;else if(item.type==='med'){if(p.kits>=5)return false;p.kits++}else if(item.type==='armor'){if(p.armor>=100)return false;p.armor=100}else return false;item.taken=true;return true}
function reload(p){const w=weapons[p.equipped],slot=p.weapons[p.equipped];if(!w||!slot)return 0;const amount=Math.min(w.mag-slot.ammo,p.reserve);slot.ammo+=amount;p.reserve-=amount;return amount}
function heal(p){if(!p.kits||p.hp>=100)return false;p.kits--;p.hp=Math.min(100,p.hp+65);return true}
return{weapons,stages,zoneAt,outsideZone,damage,newPlayer,collect,reload,heal};
});
