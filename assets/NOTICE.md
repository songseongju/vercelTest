# Character asset attribution

## Soldier / Vanguard

- Asset: `soldier.glb` (approximately 2.1 MB; textured skinned humanoid with Idle, Walk, Run and TPose clips)
- Creator/source: Mixamo (Adobe), distributed in the official three.js examples.
- Pinned upstream file: https://github.com/mrdoob/three.js/blob/r180/examples/models/gltf/Soldier.glb
- Official example credits: https://threejs.org/examples/webgl_animation_multiple.html
- Mixamo usage FAQ: https://helpx.adobe.com/creative-cloud/faq/mixamo-faq.html

Adobe's FAQ permits characters and animations to be used royalty-free in personal, commercial and non-profit projects, including video games. The model is bundled as an asset of this playable game. It is not offered as a standalone model/animation asset pack for resale or redistribution.

In-game integration: shared meshes/materials, separately cloned skeletons and animation groups, normalized 1.82 m height, procedural two-handed rifle pose, separate body/head hit volumes. The material appearance is adjusted at runtime; the upstream GLB is unmodified.

## Runtime

`vendor/babylonjs.loaders.min.js` is Babylon.js Loaders 9.26.1 (Apache-2.0), paired with the existing Babylon.js 9.26.1 engine. See `vendor/LICENSE-babylon.txt`.

## Scanned environment surfaces and daylight (v3.1)

Bundled from Poly Haven under CC0: https://polyhaven.com/license

- Asphalt 02: https://polyhaven.com/a/asphalt_02
- Concrete Wall 001: https://polyhaven.com/a/concrete_wall_001
- Grass Path 2: https://polyhaven.com/a/grass_path_2
- Bark Brown 02: https://polyhaven.com/a/bark_brown_02
- Rusty Corrugated Iron: https://polyhaven.com/a/rusty_corrugated_iron
- Kloofendal 48d Partly Cloudy Puresky: https://polyhaven.com/a/kloofendal_48d_partly_cloudy_puresky

1K diffuse, OpenGL normal and packed AO/roughness/metalness maps are in
`assets/environment/`. The 1K HDR sky is used for both backdrop and physical
material lighting. Assets are served locally with the game, with no runtime CDN.
Pine branch silhouettes are generated by the game's own canvas drawing code.

## Scanned gear surfaces (v3.2)

Bundled from Poly Haven under CC0: https://polyhaven.com/license

- Poly Wool Herringbone: https://polyhaven.com/a/poly_wool_herringbone — ballistic vest weave
- Blue Metal Plate: https://polyhaven.com/a/blue_metal_plate — helmet shell and blade steel
- Leather White: https://polyhaven.com/a/leather_white — wrapped grips

Diffuse, OpenGL normal and packed AO/roughness/metalness maps are in
`assets/gear/`, downscaled from the 1K source to 512px because the gear is small
on screen: 6.3 MB of source textures become 436 KB shipped. They are tinted and
given metallic/roughness values at runtime, so one scan serves the olive helmet,
the vest weave and the bare steel of a blade.

No image is scraped from the open web. Every bundled texture is CC0 from Poly
Haven, and the character model is covered by the Mixamo terms above.
